...
function Avatar({ name, url, size = 9 }: { name: string; url: string | null; size?: number }) {
  ...
  return (
    <div className={`${sz} rounded-full bg-[#1c2b3a] flex items-center justify-center flex-shrink-0`}>
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      const [slackResult, annResult, reqResult, toResult, contractorsResult, hoursResult, projectsResult, clientsResult, usdRateStr, invResult, linkResult] = await Promise.all([
        ...
        supabase.from('hub_users').select('id, full_name, avatar_url, payment_type, hourly_rate, monthly_rate, currency, birthday, start_date, work_days').eq('status', 'active').in('role', ['contractor', 'admin']).neq('is_developer', true),
