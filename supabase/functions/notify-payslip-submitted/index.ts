...
async function sendNotification(payout_id: string, type: 'submitted' | 'dispute' = 'submitted') {
  ...
  const HUB_BASE_URL_LOCAL = Deno.env.get('HUB_BASE_URL') ?? 'https://fsarchitects.ph';
  const payrollUrl = `${HUB_BASE_URL_LOCAL}/hub/admin/payroll`;
      ...
      <table style="width:100%;border-collapse:collapse;background:#f9fafb;border-radius:10px;overflow:hidden;">
        ...
        <tr>
          <td style="padding:14px 16px;">
            ...
            <p style="margin:4px 0 0;font-size:22px;font-weight:800;color:#1c2b3a;">${fmt(payout.final_payout)}</p>
          </td>
        </tr>
      </table>
