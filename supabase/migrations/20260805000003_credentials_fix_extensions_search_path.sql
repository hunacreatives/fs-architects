-- Real root cause of every "failed to save/load credentials" error: pgcrypto's
-- pgp_sym_encrypt/pgp_sym_decrypt live in the `extensions` schema on Supabase,
-- not `public`. These functions were declared with `set search_path = public`
-- only, so Postgres couldn't resolve pgp_sym_decrypt/pgp_sym_encrypt at all
-- ("function pgp_sym_decrypt(bytea, text) does not exist", 42883). Everything
-- else investigated along the way (schema cache, exposed-functions toggle,
-- ambiguous columns) was either a red herring or an unrelated fix that was
-- still worth having — this is the actual bug.
...
-- _cred_key() doesn't call pgcrypto itself, but fix it too for consistency —
-- it's the one function in this file that reads from vault.decrypted_secrets.
